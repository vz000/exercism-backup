...
func (t *Task) Run() {
	...
	// Delaying the execution for the sake of the example
...
func main() {
	...
	// Hold the tasks to be executed from a predefined number of goroutines
	taskQueue := make(chan *Task, 10)

	// Launch goroutines to handle the work.
	// The worker process is distributing with the value of noOfWorkers
	// Predefined number of goroutines to distribute the work
	wg.Add(noOfWorkers)
	for gr := 1; gr <= noOfWorkers; gr++ {
		go worker(taskQueue, gr)
	}

	// Add Tasks into the Buffered channel
	for i := 1; i <= 10; i++ {
		taskQueue <- &Task{
			Id: i,
			JobId: 100 + i,
