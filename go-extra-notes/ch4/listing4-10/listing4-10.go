...
func generateFibonaci(fibs chan<- fibvalue) {
	defer wg.Done()
	for i:= 1; i <= 10; i++ {
...
func printValues(fibs <-chan fibvalue, sqrs <-chan squarevalue) {
	defer wg.Done()
	for i := 1; i <= 20; i++ {
		// polls on both fibs and sqrs channels to print the resulting values
		// whenever the values can receive from both channels
		select {
		case fib := <-fibs:
